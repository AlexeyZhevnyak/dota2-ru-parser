 ...
 */
const createAnalysisPrompt = (formattedPosts: string, prompt?: string): string => {
    let basePrompt = prompt || AI_CONFIG.DEFAULT_PROMPT;

    if (!basePrompt.includes('{posts}')) {
        basePrompt = `${basePrompt}\n{posts}`;
    }

    return basePrompt.replace('{posts}', formattedPosts);
};
 ...
 */
export const analyzeUserPosts = async (posts: ParsedTopics, customPrompt?: string): Promise<string> => {
    try {
        // Validate input
        ...
        // Create the prompt
        const prompt = createAnalysisPrompt(formattedPosts, customPrompt);
